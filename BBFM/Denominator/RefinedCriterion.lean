import BBFM.Denominator.RefinedCore
...
/-- The existing denominator Fourier and cubic-Taylor estimates imply the
desired sign already at effective size10^28. The estimates themselves are
unchanged; the proof integrates the local negative part. -/
theorem quantitative_fourier_positive_half (φ : ℝ → ℂ) (hφ : Continuous φ) (N L V : ℝ)
    (hN : (10 : ℝ)^28 ≤ N) (hL : 1 ≤ L) (hLN : L ≤ N)
  ...
  have hVcube : V ≤ 100*N^3 := by
    ...
    nlinarith
  have hsmall : M^2*a^6 ≤ 1/(10 : ℝ)^10 := by
  ...
  apply positive_fourier_core (fun θ => (φ θ).re) (Complex.continuous_re.comp hφ)
    a c (Real.exp (-N/(10 : ℝ)^24)) M ha hac hcπ (Real.exp_pos _).le
  ...
  · intro θ hθ
    ...
    have hrel : ‖R‖ ≤ (V*θ^2/2)/2 := by
      have hh := local_cubic_relative N L V θ hN0 hL hVlo hθ.1 hθ.2
      linarith
    have hh := local_exponential_real_lower_error (V*θ^2/2) M θ R hM hθ.1 hb' hrel
    rw [hR,heq]
    convert hh using 1 <;> congr 2 <;> ring
  · intro θ hθ
    have hθ0 : 0 ≤ θ := hc0.trans hθ.1
    have ht : 1 ≤ θ*(100000000*L) :=
      (div_le_iff₀ (show 0 < 100000000*L by positivity)).1 hθ.1
    have ht2 := pow_le_pow_left₀ (by norm_num : (0 : ℝ) ≤ 1) ht 2
    have hmin : 1/(10 : ℝ)^16 ≤ min 1 (L^2*θ^2) := by
      apply le_min
      · norm_num
      · nlinarith only [ht2]
    ...
    have hp := mul_le_mul_of_nonneg_left hmin (show 0 ≤ N/100000000 by positivity)
    nlinarith only [hp]
...
#print axioms quantitative_fourier_positive
end BBFMRefined
