import BBFM.Denominator.RelativeCore
import BBFM.Denominator.RelativeScale
...
theorem quantitative_fourier_positive_half (φ : ℝ → ℂ) (hφ : Continuous φ) (N L V : ℝ)
  ...
  have hVcube : V ≤ 100*N^3 := by
    ...
    nlinarith
  have hsmall : M^2*a^6 ≤ 1/(10 : ℝ)^6 := by
  ...
  apply positive_fourier_core (fun θ => (φ θ).re) (Complex.continuous_re.comp hφ)
    a c (Real.exp (-N/(10 : ℝ)^9)) M ha hac hcπ (Real.exp_pos _).le
  ...
  · intro θ hθ
    ...
    have ht : 1 ≤ θ*(256*L) :=
      (div_le_iff₀ (show 0 < 256*L by positivity)).1 hθ.1
    have ht2 := pow_le_pow_left₀ (by norm_num : (0 : ℝ) ≤ 1) ht 2
    have hmin : 1/(10 : ℝ)^5 ≤ min 1 (L^2*θ^2) := by
      apply le_min
      · norm_num
      · nlinarith only [ht2]
    ...
    have hp := mul_le_mul_of_nonneg_left hmin (show 0 ≤ N/10000 by positivity)
    nlinarith only [hp]
