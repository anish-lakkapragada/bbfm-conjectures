...
theorem den_lower_interior_strict (n k : ℕ) (hn : 0 < n)
  ...
  have hbigR : 6*((10 : ℝ)^13)^2 ≤ (k : ℝ) := by exact_mod_cast hbig
  by_cases hrhalf : r ≤ 1/2
  · have hp := den_small_radius_parameters n r hn hr.le hrhalf
    dsimp only at hp
    rw [hmean] at hp
    have hN : (10 : ℝ)^13 ≤ ((Nat.log 2 n+1 : ℕ) : ℝ)*r := by linarith [hp.1]
    exact BBFMRelative.den_logconcave_of_small_saddle n k r hn hk hkd hr hrhalf hmean hN
  · have hrlarge : 1/2 < r := lt_of_not_ge hrhalf
    ...
    have hN0 : 0 ≤ N := by
      ...
      dsimp [N]
      positivity
    have hN : (10 : ℝ)^13 ≤ N := by
      apply (sq_le_sq₀ (by positivity) hN0).mp
      change _ ≤ N^2
      change (k : ℝ) ≤ 6*N^2 at hp
      nlinarith only [hbigR,hp]
    exact BBFMRelative.den_logconcave_of_large_saddle n k r hn hk hkd hrlarge hrone hmean hN
...
theorem den_interior_strict (n k : ℕ) (hn : 0 < n)
    (hleft : 6*((10 : ℕ)^13)^2 ≤ k)
    (hright : 6*((10 : ℕ)^13)^2 ≤ (den n).natDegree-k) :
    (den n).coeff (k-1)*(den n).coeff (k+1) < (den n).coeff k^2 := by
