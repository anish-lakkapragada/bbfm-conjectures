...
namespace BBFMLocal
open Real intervalIntegral BBFMRelative


lemma cosine_weight_small (θ : ℝ) (hθ : 0 ≤ θ) (hθ1 : θ ≤ 1) :
    9*θ^2/20 ≤ 1-Real.cos θ := by
  have hs := Real.sin_ge_sub_cube (show 0 ≤ θ/2 by linarith)
  have hθ2 : θ^2 ≤ 1 := by nlinarith
  have hθ3 : θ^3 ≤ θ := by nlinarith [mul_le_mul_of_nonneg_right hθ2 hθ]
  have hs' : 23*θ/48 ≤ Real.sin (θ/2) := by nlinarith
  have hsq := (sq_le_sq₀ (by positivity) (by linarith : 0 ≤ Real.sin (θ/2))).mpr hs'
  have hid := Real.sin_sq_eq_half_sub (θ/2)
  rw [show 2*(θ/2)=θ by ring] at hid
  nlinarith

/-- A Gaussian eighth-moment bound controls the local negative contribution
to the Fourier integral when normalized squared error is at most 10^(-5). -/
theorem positive_fourier_core (f : ℝ → ℝ) (hf : Continuous f) (a c δ M : ℝ)
    (ha : 0 < a) (ha1 : a ≤ 1) (hac : a ≤ c) (hcπ : c ≤ Real.pi)
    (hδ : 0 ≤ δ) (hδsmall : δ ≤ a^3/10000)
    (hsmall : M^2*a^6 ≤ 1/(10 : ℝ)^5)
  ...
  have hi : 3*a^3/80 ≤ ∫ θ in (0 : ℝ)..a, g θ := by
    have hh := integral_mono_on (μ := MeasureTheory.volume) ha.le
      ((by fun_prop : Continuous (fun θ : ℝ => 9*θ^2/80)).intervalIntegrable _ _)
      (hg.intervalIntegrable _ _)
      (fun θ (hθ : θ ∈ Set.Icc (0 : ℝ) a) => ?_)
    · have hei : (∫ θ in (0 : ℝ)..a, 9*θ^2/80) = 3*a^3/80 := by
        rw [integral_div, integral_const_mul, integral_pow]
        norm_num
        ring
      rwa [hei] at hh
    · dsimp [g]
      have hw := cosine_weight_small θ hθ.1 (hθ.2.trans ha1)
      have hw0 : 0 ≤ 1-Real.cos θ := sub_nonneg.mpr (Real.cos_le_one θ)
      have hp := mul_le_mul_of_nonneg_right (hinner θ hθ) hw0
      nlinarith
  have hm : -(3*a^3/100) ≤ ∫ θ in a..c, g θ := by
    have hh := integral_mono_on (μ := MeasureTheory.volume) hac
      ((by fun_prop : Continuous (fun θ : ℝ => -M^2/2*e θ)).intervalIntegrable _ _)
      (hg.intervalIntegrable _ _)
    ...
    · rw [integral_const_mul] at hh
      ...
      change (∫ θ in (0 : ℝ)..c, e θ) ≤ 6000*a^9 at hb
      have hb' : (∫ θ in a..c, e θ) ≤ 6000*a^9 := by linarith
...
#print axioms BBFMLocal.positive_fourier_core
end BBFMLocal
