import BBFM.Denominator.RefinedSaddles
import BBFM.Denominator.Analytic.LinearEdge

/-! Coverage and reflection combining the elementary edge theorem and the RefinedSaddles threshold. -/

noncomputable section
namespace BBFMRefined
open Polynomial Real DenominatorResearch
...
theorem den_lower_half_logconcave (n k : ℕ) (hn : 0 < n) (hk : 1 ≤ k)
  ...
  have hklarge := effective_parameter_coverage ((Nat.log 2 n + 1 : ℕ) : ℝ) (k : ℝ)
    ((10 : ℝ) ^ 28) (by positivity) htR heR (by positivity)
  ...
  · have hpar := den_small_radius_parameters n r hn hr.le hrhalf
    dsimp only at hpar
    rw [hmean] at hpar
    have hN : (10 : ℝ) ^ 28 ≤ ((Nat.log 2 n + 1 : ℕ) : ℝ) * r := by linarith [hpar.1]
    exact (BBFMRefined.den_logconcave_of_small_saddle n k r hn hk hkd hr hrhalf hmean hN).le
  · have hrlarge : 1 / 2 < r := lt_of_not_ge hrhalf
    ...
    have hN : (10 : ℝ) ^ 28 ≤
      ...
      have hc : (10 : ℝ) ^ 28 ≤ 100 * ((10 : ℝ) ^ 28) ^ 2 / 2 := by norm_num
      linarith
    exact (BBFMRefined.den_logconcave_of_large_saddle n k r hn hk hkd hrlarge hrone hmean hN).le
...
theorem den_logconcave (n k : ℕ) (hn : 0 < n) (hk : 1 ≤ k)
  ...
  · by_cases hmid : 2 * k ≤ (den n).natDegree
    · exact BBFMRefined.den_lower_half_logconcave n k hn hk hmid hlarge
    · let j := (den n).natDegree - k
      have hj : 1 ≤ j := by dsimp [j]; omega
      have hjmid : 2 * j ≤ (den n).natDegree := by dsimp [j]; omega
      have hh := BBFMRefined.den_lower_half_logconcave n j hn hj hjmid hlarge
...
provided E >= 10^58. E is symbolic so the closed power of two is never evaluated. -/
theorem den_logconcave_of_pow_le (n k E : ℕ) (hk : 1 ≤ k)
    (hE : 100 * ((10 : ℕ) ^ 28) ^ 2 ≤ E) (hlarge : 2 ^ E ≤ n) :
    (den n).coeff (k - 1) * (den n).coeff (k + 1) ≤ (den n).coeff k ^ 2 := by
  have hn : 0 < n := lt_of_lt_of_le (pow_pos (by decide : 0 < (2 : ℕ)) E) hlarge
  apply BBFMRefined.den_logconcave n k hn hk
...
theorem den_eventually_logconcave :
    ∃ n₀ : ℕ, ∀ n : ℕ, n₀ ≤ n → ∀ k : ℕ, 1 ≤ k →
      (den n).coeff (k - 1) * (den n).coeff (k + 1) ≤ (den n).coeff k ^ 2 := by
  refine ⟨2 ^ (100 * ((10 : ℕ) ^ 28) ^ 2), ?_⟩
  intro n hn k hk
  exact BBFMRefined.den_logconcave_of_pow_le n k _ hk le_rfl hn

#print axioms den_logconcave
#print axioms den_eventually_logconcave
end BBFMRefined
